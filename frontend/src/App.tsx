...
interface EpisodeIntakeQueueItem {
  episode: Episode
...
interface BillingFollowUpSection {
...
interface RoleDashboardSection {
  title: string
  description: string
  items: RoleWorkItem[]
}

interface QaAssignmentDraft {
...
function normalizeDocumentationPayload(
...
function buildAdmissionSnapshotFromReferral(referral: Referral): EpisodeAdmissionSnapshot {
  return {
...
function buildDocumentationQaTasksForReferral(
  episodeId: number,
  referral: Referral,
  startingId: number,
  certStart: string,
...
function buildDemoPhysicianOrder(
  dataset: AppDataset,
  episode: Episode,
  payload: {
    order_scope: string
...
function buildDemoPhysicianOrderDraft(dataset: AppDataset, episode: Episode, scope: string) {
  const assessment = dataset.assessments
...
function normalizeDemoReferralOrderStatus(orderStatus?: string, isSigned?: boolean) {
  if (isSigned) {
    return 'signed'
  }
...
function buildEpisodeIntakeQueue(episodes: Episode[], referrals: Referral[], qaTasks: QaTask[]): EpisodeIntakeQueueItem[] {
  return qaTasks
...
function buildClaimReadinessQueue(dataset: AppDataset): ClaimReadinessItem[] {
...
function buildClaimStatusLanes(items: ClaimReadinessItem[]): ClaimStatusLane[] {
  return [
    {
      key: 'submitted',
      title: 'Submitted',
...
function buildDenialQueueSections(items: ClaimReadinessItem[]): DenialQueueSection[] {
...
function buildEvvQueueSections(dataset: AppDataset): EvvQueueSection[] {
...
function buildBillingFollowUpSections(
  dataset: AppDataset,
...
function normalizeAssessmentPayload(value?: Assessment['assessment_payload'] | string | null) {
  if (!value) {
    return null
  }
...
function buildVisitRecommendations(
  episode: Episode,
  snapshot: EpisodeAdmissionSnapshot | null,
  visits: Visit[],
): VisitRecommendation[] {
...
function buildWeekOneFrequencyPlan(
  episode: Episode,
  snapshot: EpisodeAdmissionSnapshot | null,
  visits: Visit[],
): VisitRecommendation[] {
...
function computeDemoReadiness(episodeId: number, dataset: AppDataset): EpisodeReadiness {
...
function buildDemoEpisodeReviewSummary(episode: Episode, dataset: AppDataset): EpisodeReviewSummary {
...
function buildAdminReportSummary(dataset: AppDataset, period: 'last_7' | 'last_30' | 'all'): AdminReportSummary {
...
function normalizeQaTasksForUi(tasks: QaTask[]) {
  return tasks
...
function normalizeQaTaskHistory(history?: Array<Record<string, string>> | string) {
...
function buildDocumentationSummary(payload: {
  visit_focus?: string
